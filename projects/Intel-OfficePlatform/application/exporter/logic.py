...
from application.recorder import Recorder
...
def get_database_config():
    config = ConfigParser()
    config.read(filenames=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.ini'), encoding='utf-8')
...
def get_sql(tableName='ckmx'):
    ...
    sqls = {
        ...
        'hccb': config.get('sqls', 'hccb'),
        'lrtz': config.get('sqls', 'lrtz'),
        'fpsr': config.get('sqls', 'fpsr'),
        'lrmx': config.get('sqls', 'lrmx'),
        'lrhz': config.get('sqls', 'lrhz'),
        'hthklr': config.get('sqls', 'hthklr'),
        'tzclcb': config.get('sqls', 'tzclcb'),
    }
...
def generate_excel(tableName='ckmx', documentName='excel'):
    ...
    wb.save(os.path.join(os.path.dirname(os.path.abspath(__file__)), f'./{documentName}.xlsx'))
